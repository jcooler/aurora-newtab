...
describe('code-backed privacy inventory', () => {
  ...
  it('discloses encrypted sync without overstating its recoverable key model', () => {
    ...
    expect(ACCOUNT_SERVICE_DATA_FLOWS.sync.currentState).toContain('hosted sync authority is active')
    expect(ACCOUNT_SERVICE_DATA_FLOWS.sync.currentState).toContain('explicit enablement')
    expect(privacyPolicy).toContain('Encrypted sync is optional and starts only after you turn it on')
    expect(privacyPolicy).toContain('hosted production sync authority is active')
