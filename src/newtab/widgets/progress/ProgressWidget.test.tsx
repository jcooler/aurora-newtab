...
describe('Progress canvas rail content and actions', () => {
  ...
  it('renders at most three manual-first rows and an exact quiet overflow route', async () => {
    ...
    expect(route.className).toContain('[@media(max-width:640px)]:opacity-100')
    expect(route.className).toContain('min-h-8')
    expect(route.className).toContain('[@media(pointer:coarse)]:min-h-9')
    expect(route.className).toContain('[@media(max-width:640px)]:min-h-9')
    fireEvent.focus(rows[0]!)
    expect(route.className).toContain('opacity-100')
    expect(route.className).not.toContain('opacity-0')
  ...
  it('retries the exact manual intent against fresh rows and the rolled-over local day', async () => {
    ...
    expect(retry.className).toContain('min-w-9')
    expect(retry.className).toContain('px-2')
