...
import { createProviderHandlers } from './providerAuth.ts'
import {
  createProviderCrypto,
  decodeProviderBase64Url,
  encodeProviderBase64Url,
} from './providerCrypto.ts'
import { createProviderGoogleGateway, providerSha256Base64Url } from './providerGoogle.ts'
...
export async function createRuntimeProviderHandlers(environment: RuntimeEnvironment) {
  ...
  const oauthClientId = required(environment, 'GOOGLE_CALENDAR_OAUTH_CLIENT_ID')
  const oauthClientSecret = required(environment, 'GOOGLE_CALENDAR_OAUTH_CLIENT_SECRET')
  ...
  return createProviderHandlers({
    ...
    google: createProviderGoogleGateway({ clientId: oauthClientId, clientSecret: oauthClientSecret }),
    now: Date.now,
    randomUUID: () => crypto.randomUUID(),
    randomBytes(length) {
      return crypto.getRandomValues(new Uint8Array(length))
    },
    hash: providerSha256Base64Url,
    ...
    oauthClientId,
    oauthCallbackUrl: `${supabaseUrl.replace(/\/+$/u, '')}/functions/v1/google-calendar-oauth-callback`,
