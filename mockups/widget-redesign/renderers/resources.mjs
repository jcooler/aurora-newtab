...
const homeBody = (tier, f) => `<div class="home-face" data-resource-signature="homeassistant">${f.entities.slice(0, limits(tier, 1, 3, 4)).map((x) => `<article data-entity-state><div><strong>${safe(x.name)}</strong><span>${safe(x.detail)}</span></div><b>${safe(x.state)}</b>${tier === 'standard' || tier === 'full' ? '<button type="button">Open</button>' : ''}</article>`).join('')}</div>`
