...
export function renderCalendarConsolidation(fixture) {
  return `<section class="calendar-consolidation" data-calendar-consolidation><header><span>Calendar consolidation</span><h3>Choose which placement to keep.</h3><p>Your feeds, holiday country, and view settings move with the placement you choose.</p></header><div class="calendar-consolidation__placements">${fixture.placements.map((placement) => `<button type="button" data-calendar-placement><strong>${safe(placement.name)}</strong><span>${safe(placement.tier)} · ${safe(placement.position)}</span><small>${safe(placement.detail)}</small></button>`).join('')}</div><footer><button type="button">Later</button><button type="button">Save</button></footer></section>`
}
