# NL-P5 Tier Catalog — Batch 1

Owner review per the named-layouts spec §2.3: each widget, each supported
tier, judged as a designed composition under the no-whitespace law. Docked
lines are one dense text-first row (middle dots separate facts). Captures
were taken from the production preview build at 1600x900 with seeded data.

Batch-1 notes for the review:
- Timer/Tasks/Notes Docked lines are their existing dense launcher chips, declared rather than rebuilt.
- Bookmarks Docked is the full readable bar (spec exemption).
- Focus Docked is its existing single-line form rendered in the strip.
- greeting, search, and quote declare NO Docked tier in batch 1 (no honest one-line dock form); overrule here if wanted.

## Clock

| Tier | Content contract | Capture | Owner verdict |
| --- | --- | --- | --- |
| compact | Current time | ![clock compact](clock-compact.png) | _pending_ |
| standard | Time and date | ![clock standard](clock-standard.png) | _pending_ |
| full | Large, legible time and date | ![clock full](clock-full.png) | _pending_ |
| docked | Time · date | ![clock docked](clock-docked.png) | _pending_ |

## Greeting

| Tier | Content contract | Capture | Owner verdict |
| --- | --- | --- | --- |
| compact | Greeting | ![greeting compact](greeting-compact.png) | _pending_ |
| standard | More legible greeting | ![greeting standard](greeting-standard.png) | _pending_ |

## Search

| Tier | Content contract | Capture | Owner verdict |
| --- | --- | --- | --- |
| compact | Search action | ![search compact](search-compact.png) | _pending_ |
| standard | More legible search action | ![search standard](search-standard.png) | _pending_ |

## Focus

| Tier | Content contract | Capture | Owner verdict |
| --- | --- | --- | --- |
| compact | Focus action | ![focus compact](focus-compact.png) | _pending_ |
| standard | Focus detail | ![focus standard](focus-standard.png) | _pending_ |
| docked | Focus text and completion | ![focus docked](focus-docked.png) | _pending_ |

## Quote

| Tier | Content contract | Capture | Owner verdict |
| --- | --- | --- | --- |
| compact | Quote | ![quote compact](quote-compact.png) | _pending_ |
| standard | Readable full quote | ![quote standard](quote-standard.png) | _pending_ |

## Weather

| Tier | Content contract | Capture | Owner verdict |
| --- | --- | --- | --- |
| compact | Current temperature and condition | ![weather compact](weather-compact.png) | _pending_ |
| standard | Forecast context | ![weather standard](weather-standard.png) | _pending_ |
| full | Detailed forecast | ![weather full](weather-full.png) | _pending_ |
| docked | Temperature · location · condition | ![weather docked](weather-docked.png) | _pending_ |

## Timer

| Tier | Content contract | Capture | Owner verdict |
| --- | --- | --- | --- |
| compact | Timer action | ![timer compact](timer-compact.png) | _pending_ |
| docked | Timer state | ![timer docked](timer-docked.png) | _pending_ |

## Tasks

| Tier | Content contract | Capture | Owner verdict |
| --- | --- | --- | --- |
| compact | Tasks action | ![tasks compact](tasks-compact.png) | _pending_ |
| docked | Tasks action | ![tasks docked](tasks-docked.png) | _pending_ |

## Notes

| Tier | Content contract | Capture | Owner verdict |
| --- | --- | --- | --- |
| compact | Notes action | ![notes compact](notes-compact.png) | _pending_ |
| docked | Notes action | ![notes docked](notes-docked.png) | _pending_ |

## Bookmarks

| Tier | Content contract | Capture | Owner verdict |
| --- | --- | --- | --- |
| compact | Bookmark marks | ![bookmarks compact](bookmarks-compact.png) | _pending_ |
| standard | Named bookmark bar | ![bookmarks standard](bookmarks-standard.png) | _pending_ |
| docked | Full readable bookmark bar | ![bookmarks docked](bookmarks-docked.png) | _pending_ |
