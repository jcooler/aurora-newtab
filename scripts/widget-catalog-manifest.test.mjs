...
const EXPECTED_IDS = [
  ...
  'greeting', 'habits', 'homeassistant', 'ics', 'jira', 'linear', 'links', 'metrics', 'monthCal',
...
test('covers all 38 identities exactly once across disjoint catalog batches', () => {
...
test('returns the declared capture tiers without deriving presentation', () => {
  ...
  assert.deepEqual(captureTiersFor('onThisDay'), ['compact', 'standard', 'full', 'docked'])
  assert.deepEqual(captureTiersFor('auroraKp'), ['compact', 'standard', 'full', 'docked'])
