...
| 2. Token connectors (GitHub, GitLab, Jira, Vercel, crypto, ICS calendar) | `docs/superpowers/specs/2026-08-02-token-connectors-design.md` | `docs/superpowers/plans/2026-08-07-token-connectors.md` (tasks 46–55, DONE) | **DONE 2026-08-08 at `11e1b4f`** — v1.4.0 staged (NOT submitted; v1.2.1 verdict pending); 931 tests, 178 harness assertions; fable review Satisfied-with-fixes, wave applied+verified. SP3 inherits: z-30 stacking discipline applies only at DEFAULT placement (PositionedBlock drops className on user-arranged pos — pre-existing); ≤800px-height band vertical rhythm follow-up (index.css); post-import ICS card could hint "paste your address again". |
| 3′. NEXT → OAuth wave below, or SP4 local widgets (either order works; SP4 has no external paperwork) | | | **START HERE next** |
...
| 4. Local widgets (habit streaks, month calendar) | `docs/superpowers/specs/2026-08-02-local-widgets-design.md` | `docs/superpowers/plans/2026-08-08-local-widgets.md` (tasks 56–59, DONE) | **DONE 2026-08-08 at `e7a4a30`** — v1.5.0 staged (v1.2.1 verdict still gates); 1005 tests, 201 harness assertions, 190-pair gate; fable review Satisfied-with-fixes, wave applied+verified. Schema now v7 (habits + generic nested-widgets migration; the bump rule is documented ON WidgetToggles in schema.ts). SP3 inherits: cursor-pointer missing on monthCal nav buttons (one-line warm-up); ≤800px band rhythm follow-up still open. **SP3 (OAuth) is the only sub-project left.** |
